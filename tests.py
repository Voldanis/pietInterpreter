...
class TestMainScriptTrueExecution(unittest.TestCase):
    """Качественные интеграционные и E2E тесты для точки входа main.py."""

    @classmethod
    def setUpClass(cls):
        """
        Подготовка окружения. Создаем временную папку Gallery_backup, 
        если оригинальная папка существует, чтобы изолировать тесты от реальных картинок,
        либо создаем чистую временную папку Gallery для воспроизводимости.
        """
        cls.gallery_dir = "Gallery"
        cls.backup_dir = "Gallery_backup"
        
        # Если папка Gallery существует, временно переименовываем её
        if os.path.exists(cls.gallery_dir):
            if os.path.exists(cls.backup_dir):
                shutil.rmtree(cls.backup_dir)
            os.rename(cls.gallery_dir, cls.backup_dir)
            cls.had_original_gallery = True
        else:
            cls.had_original_gallery = False
            
        os.makedirs(cls.gallery_dir, exist_ok=True)

        # Список файлов, которые жестко затребованы в текущей версии main.py:
        # three.png, one.png, hw1-11.gif, hw3-5.gif, pc.png, one_bright.png, helloworld-pietbig.gif, alpha_filled_big.png
        cls.required_files = [
            "three.png", "one.png", "hw1-11.gif", "hw3-5.gif", 
            "pc.png", "one_bright.png", "helloworld-pietbig.gif", "alpha_filled_big.png"
        ]
        
        # Генерируем минимальные валидные одноцветные изображения (1x1 пиксель, красный цвет)
        # Этого достаточно, чтобы нормализаторы и загрузчики PIL/NumPy не падали,
        # и интерпретатор Piet мог выполнить happy-path инициализацию и запуск.
        for filename in cls.required_files:
            file_path = os.path.join(cls.gallery_dir, filename)
            img = Image.new("RGB", (1, 1), color=(255, 0, 0)) # Красный кодел
            img.save(file_path)

    @classmethod
    def tearDownClass(cls):
        """Восстановление исходного состояния репозитория после тестов."""
        # Удаляем временную папку со сгенерированными тестами
        if os.path.exists(cls.gallery_dir):
            shutil.rmtree(cls.gallery_dir)
            
        # Возвращаем оригинальную папку Gallery на место
        if cls.had_original_gallery and os.path.exists(cls.backup_dir):
            os.rename(cls.backup_dir, cls.gallery_dir)

    def test_main_e2e_execution_success(self):
        """
        E2E Тест: Проверяет сквозной запуск main.py как независимого подпроцесса.
        Гарантирует отсутствие необработанных исключений (Crash-test).
        """
        # Запускаем main.py в том же окружении Python, где запущены тесты
        ...
        # Проверяем код возврата. 0 означает, что скрипт успешно выполнился до конца
        self.assertEqual(
            result.returncode, 0, 
            f"Скрипт main.py упал во время исполнения!\n"
            f"STDOUT:\n{result.stdout}\n"
            f"STDERR:\n{result.stderr}"
        )

    def test_main_handles_missing_gallery_directory(self):
        """
        Негативный тест: Проверяет поведение системы, если папка Gallery отсутствует.
        Помогает увидеть, падает ли приложение с понятной ошибкой или сырым трейсбэком.
        """
        # Временно ломаем имя папки
        temp_broken_dir = "Gallery_broken"
        os.rename(self.gallery_dir, temp_broken_dir)
        ...
        try:
            ...
            # Здесь мы ожидаем, что скрипт завершится с ошибкой (код != 0),
            # так как встроенная логика dop_funcs или PIL не найдет файлы.
            self.assertNotEqual(
                result.returncode, 0, 
                "main.py вернул 0 (успех) при отсутствующей папке Gallery, хотя должен был упасть."
            )
            # Дополнительно можно проверить наличие слова FileNotFoundError в ошибке
            self.assertTrue(
                "FileNotFoundError" in result.stderr or "OSError" in result.stderr,
                f"Неожиданный лог ошибки при отсутствии папки:\n{result.stderr}"
            )
        finally:
            # В любом случае возвращаем всё назад
            os.rename(temp_broken_dir, self.gallery_dir)
